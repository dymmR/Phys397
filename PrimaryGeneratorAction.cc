...
: G4VUserPrimaryGeneratorAction(),fParticleGun()
{
  ...
  fParticleGun->SetParticlePosition(G4ThreeVector(19.*cm,0.*cm,29.*cm));
  ...
  fParticleGun->SetParticleEnergy(11*keV);
  fParticleGun->SetParticlePosition(G4ThreeVector(7.*cm,0.*cm,14.*cm));
  fParticleGun->SetParticleMomentumDirection(G4ThreeVector(sinTheta*cos(phi),
							    sinTheta*sin(phi),
							    cosTheta));
}
